...
* NEW❗: Dynamic Longitudinal Control (DLC) thanks to dragonpilot-community!
  ...
  * Dynamic Longitudinal Control must be enabled via "SP - Controls", then toggle via the button on the driving screen, while using Experimental Mode
  * When Dynamic Longitudinal Control is ON, initially setting cruise speed will set to the vehicle's current speed
