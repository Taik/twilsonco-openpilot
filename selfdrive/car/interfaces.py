...
class CarInterfaceBase(ABC):
  ...
  def get_acc_mads(self, cruise_state_enabled, acc_enabled):
    if self.acc_mads_combo:
      if not self.prev_acc_mads_combo and (cruise_state_enabled or acc_enabled):
        self.madsEnabled = True
      self.prev_acc_mads_combo = (cruise_state_enabled or acc_enabled)

  def get_sp_v_cruise_non_pcm_state(self, cs_out, button_events, vCruise,
                                    enable_buttons=(ButtonType.accelCruise, ButtonType.decelCruise),
  ...
  def get_sp_cruise_main_state(self, cruise_state_available):
    if not self.CS.control_initialized:
      mads_enabled = False
    elif not self.mads_main_toggle:
      mads_enabled = False
    else:
      mads_enabled = cruise_state_available

    return mads_enabled

  def get_sp_common_state(self, cs_out, min_enable_speed_pcm=False, gear_allowed=True, gap_button=False):
    cs_out.cruiseState.enabled = self.accEnabled if not self.CP.pcmCruise or not self.CP.pcmCruiseSpeed or min_enable_speed_pcm else \
    ...
    if not self.enable_mads and self.CP.pcmCruise:
      if not cs_out.cruiseState.enabled and cs_out.madsEnabled:
        cs_out.madsEnabled = False

    cs_out.madsEnabled = self.madsEnabled
    cs_out.accEnabled = self.accEnabled
    cs_out.disengageByBrake = self.disengageByBrake
  ...
  def get_sp_gac_state(self, gac_tr, gac_min, gac_max, inc_dec):
    if self.op is None:
      self.op = self.op_lookup.get(inc_dec)
    gac_tr = self.op(gac_tr, 1)
    if inc_dec == "+":
  ...
  def toggle_gac(self, gac_button, gac_min, gac_max, gac_default, inc_dec):
    if not self.CP.openpilotLongitudinalControl:
      if self.CS.gac_tr_cluster != gac_default:
        self.CS.gac_tr_cluster = gac_default
      if self.CS.gac_tr != 2:
        self.CS.gac_tr = 2
    ...
    if gac_button:
      ...
      self.gac_button_counter = 0
      self.CS.gac_tr = self.get_sp_gac_state(self.CS.gac_tr, 0, 2, inc_dec)
      put_nonblocking("LongitudinalPersonality", str(self.CS.gac_tr))
    else:
      self.gac_button_counter = 0
    if self.prev_gac_tr != self.CS.gac_tr:
      self.CS.gac_tr_cluster = clip(self.CS.gac_tr + 1, gac_min, gac_max)  # always 1 higher
    self.prev_gac_tr = self.CS.gac_tr
  ...
  def create_sp_events(self, cs_out, events, main_enabled=False, allow_enable=True, enable_pressed=False,
    ...
    if self.CP.pcmCruise:
      ...
      if cs_out.cruiseState.enabled and not self.CS.out.cruiseState.enabled and allow_enable:
        enable_pressed = True
        enable_pressed_long = True
      elif not cs_out.cruiseState.enabled and not cs_out.madsEnabled:
        events.add(EventName.buttonCancel)
    if enable_pressed:
  ...
  def sp_update_params(self):
    self.experimental_mode = self.param_s.get_bool("ExperimentalMode")
    self.CS.gac_tr = int(self.param_s.get("LongitudinalPersonality"))
    self._frame += 1
